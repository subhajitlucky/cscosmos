...
import { ArrowLeft, ArrowRight, Box, CheckCircle2, HelpCircle, Lightbulb, Link as LinkIcon, RotateCcw } from 'lucide-react';
import { BLOCKCHAIN_FLASHCARDS, type BlockchainFlashcard } from '../data/flashcards';
...
export default function Flashcards() {
  ...
  const filteredCards = selectedCat === 'All'
    ? BLOCKCHAIN_FLASHCARDS
    : BLOCKCHAIN_FLASHCARDS.filter((c) => c.category === selectedCat);
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-bold uppercase tracking-widest">
          <LinkIcon className="w-3.5 h-3.5" /> Spaced Recall Trainer
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">
          Web3 &amp; Solidity Senior Interview Flashcards
        </h1>
        <p className="text-muted-foreground text-sm sm:text-base">
          Master tough Blockchain, EVM, and Smart Contract Security questions on SSTORE gas costs, reentrancy guards, and ZK-Rollups.
        </p>
      </div>

      {/* Category Chips */}
      <div className="flex flex-wrap justify-center gap-2">
        {['All', 'Cryptography & Hashing', 'Consensus & P2P', 'EVM & Smart Contracts', 'Security & Scaling'].map((cat) => (
          <button
            key={cat}
            onClick={() => {
              setSelectedCat(cat);
      ...
      <div
        ...
        className="min-h-[360px] p-8 rounded-3xl border border-border bg-card hover:border-amber-500/50 shadow-xl cursor-pointer transition-all flex flex-col justify-between select-none relative group"
      >
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs font-bold uppercase">
          ...
          {!isFlipped ? (
            <div className="space-y-3">
              <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                <HelpCircle className="w-4 h-4 text-amber-500" /> Question:
              </div>
            ...
            <div className="space-y-4 animate-in fade-in">
              <div className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Answer:
              </div>
              <p className="text-sm sm:text-base text-foreground leading-relaxed font-medium">
                {currentCard.answer}
              </p>
              {currentCard.code && (
                <pre className="p-3.5 rounded-2xl bg-slate-950 text-amber-300 font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed">
                  {currentCard.code}
                </pre>
              )}
        ...
        <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="text-amber-500 font-bold group-hover:underline">Flip Card 🔄</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-2">
        ...
        <button
          onClick={handleNext}
          className="px-6 py-2.5 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition shadow-md flex items-center gap-1.5"
        >
          <span>Next Card</span>
