...
import { BLOCKCHAIN_TOPICS } from '../data/topics';

export default function ConceptDetail({ topicId }: { topicId: string }) {
  const topic = BLOCKCHAIN_TOPICS.find((t) => t.id === topicId);
  ...
  const nextTopic = topic.nextTopicId
    ? BLOCKCHAIN_TOPICS.find((t) => t.id === topic.nextTopicId)
    : null;
  ...
  return (
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      ...
      <div className="p-6 rounded-3xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 space-y-2">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-muted-foreground font-semibold">
          <span className="flex items-center gap-1.5">
            <Terminal className="w-4 h-4 text-amber-500" /> Solidity / TypeScript / Bytecode
          </span>
          <span className="font-mono text-[11px]">Production Standard</span>
        </div>
        <pre className="p-5 rounded-3xl bg-slate-950 text-amber-300 font-mono text-xs overflow-x-auto border border-border shadow-inner whitespace-pre-wrap leading-relaxed">
          {topic.codeSnippet}
        </pre>
      </div>
      ...
      {nextTopic && (
        <div className="pt-6 border-t border-border flex justify-end">
          <Link
            href={`/blockchainviz/concepts/${nextTopic.id}`}
            className="px-6 py-3 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs uppercase tracking-wider transition shadow-md flex items-center gap-2"
          >
