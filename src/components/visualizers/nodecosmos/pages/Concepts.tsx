...
import { NODE_TOPICS, type NodeTopic } from '../data/topics';

export default function Concepts() {
  const [filter, setFilter] = useState<string>('all');

  const filtered = filter === 'all' ? NODE_TOPICS : NODE_TOPICS.filter((t) => t.category === filter);
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto px-4 sm:px-6 pt-10">
      ...
      <div className="flex flex-wrap gap-2">
        {['all', 'foundations', 'event-loop', 'streams', 'concurrency', 'internals'].map((cat) => (
          <button
            key={cat}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((topic) => (
          <Link
            key={topic.id}
            href={`/nodecosmos/concepts/${topic.id}`}
