...
import { ArrowRight, BookOpen, Filter, Server, Sparkles } from 'lucide-react';
import { SYSTEM_DESIGN_TOPICS, type SystemDesignTopic } from '../data/topics';
...
export default function Concepts() {
  ...
  const filtered = filter === 'all'
    ...
    : SYSTEM_DESIGN_TOPICS.filter((t) => t.category === filter);
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto px-4 sm:px-6 pt-10">
      ...
      <div className="flex flex-wrap gap-2">
        {['all', 'distributed-data', 'consensus', 'availability', 'caching-queues', 'resilience', 'scaling'].map((cat) => (
          <button
            key={cat}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((topic) => (
          <Link
            key={topic.id}
            href={`/systemdesignviz/concepts/${topic.id}`}
