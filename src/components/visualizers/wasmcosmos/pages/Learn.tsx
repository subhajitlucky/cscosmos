...
import { Search, Cpu, BookOpen, ArrowUpRight } from 'lucide-react';
import { wasmTopicGroups, wasmTopics } from '../data/topics';
...
export function Learn() {
  ...
  const filteredGroups = useMemo(() => {
    return wasmTopicGroups
      ...
      .filter(Boolean) as typeof wasmTopicGroups;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-16">
      ...
      <div className="space-y-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[var(--wasm-primary)]/30 bg-[var(--wasm-primary)]/10 text-[var(--wasm-primary)] text-xs font-mono">
          <BookOpen className="w-3.5 h-3.5" /> WebAssembly Systems Curriculum
        </div>

        <h1 className="font-display text-4xl md:text-6xl font-extrabold tracking-tight text-[var(--wasm-text)]">
          The WebAssembly <span className="text-[var(--wasm-primary)] wasm-glow">Architecture Map</span>
        </h1>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 pt-4">
          <div className="relative flex-grow w-full md:w-auto">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--wasm-muted)]" />
            <input
              type="text"
              placeholder="Search concepts (e.g. stack, linear memory, wat, wasi, component, rust)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-[var(--wasm-border-subtle)] bg-[var(--wasm-surface)] text-xs font-mono text-[var(--wasm-text)] placeholder:text-[var(--wasm-muted)] focus:outline-none focus:border-[var(--wasm-primary)]"
            />
          </div>
            ...
            <div className="flex items-center justify-between border-b border-[var(--wasm-border-subtle)] pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs font-bold text-[var(--wasm-primary)]">
                    TRACK_0{groupIdx + 1}
                  </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {group.topics.map((topic) => (
                <Link
                  ...
                  className="wasm-card p-6 rounded-xl space-y-4 group flex flex-col justify-between"
                >
                  ...
                  <div className="pt-4 border-t border-[var(--wasm-border-subtle)] flex items-center justify-between">
                    <span className="text-[10px] font-mono text-[var(--wasm-muted)]">
                      {topic.difficulty}
                    </span>
                    <span className="text-[10px] font-mono text-[var(--wasm-primary)] flex items-center gap-1">
