...
export default function Flashcards() {
  ...
  const filteredCards = selectedCat === 'All'
    ? REDIS_FLASHCARDS
    : REDIS_FLASHCARDS.filter((c) => c.category === selectedCat);
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-xs font-bold uppercase tracking-widest">
          <Database className="w-3.5 h-3.5" /> Spaced Recall Trainer
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">
          Redis Senior Interview Flashcards
        </h1>
        <p className="text-muted-foreground text-sm sm:text-base">
          Master the single-threaded event loop, SkipList rankings, Copy-on-Write snapshots, and caching failure modes.
        </p>
      </div>

      {/* Category Chips */}
      <div className="flex flex-wrap justify-center gap-2">
        {['All', 'Foundations', 'Data Structures', 'Caching & Eviction', 'Persistence & Scale'].map((cat) => (
          <button
            key={cat}
            onClick={() => {
              setSelectedCat(cat);
      ...
      <div
        ...
        className="min-h-[360px] p-8 rounded-3xl border border-border bg-card hover:border-red-500/50 shadow-xl cursor-pointer transition-all flex flex-col justify-between select-none relative group"
      >
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-red-500/10 text-red-600 dark:text-red-400 text-xs font-bold uppercase">
          ...
          {!isFlipped ? (
            <div className="space-y-3">
              <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                <HelpCircle className="w-4 h-4 text-red-500" /> Question:
              </div>
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              {currentCard.code && (
                <pre className="p-3.5 rounded-2xl bg-slate-950 text-red-300 font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed">
                  {currentCard.code}
                </pre>
              )}
        ...
        <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="text-red-500 font-bold group-hover:underline">Flip Card 🔄</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-2">
        ...
        <button
          onClick={handleNext}
          className="px-6 py-2.5 rounded-2xl bg-red-600 hover:bg-red-700 text-white text-xs font-bold transition shadow-md flex items-center gap-1.5"
        >
          <span>Next Card</span>
