...
import { AlertCircle, ArrowLeft, ArrowRight, BookOpen, CheckCircle2, Code2, Sparkles, Terminal, XCircle } from 'lucide-react';
import { REDIS_TOPICS } from '../data/topics';

export default function ConceptDetail({ topicId }: { topicId: string }) {
  const topic = REDIS_TOPICS.find((t) => t.id === topicId);
  ...
  const nextTopic = topic.nextTopicId
    ? REDIS_TOPICS.find((t) => t.id === topic.nextTopicId)
    : null;
  ...
  return (
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      ...
      >
        <ArrowLeft className="w-3.5 h-3.5" /> Back to 18 Concepts
      </Link>
      ...
      <div className="p-6 rounded-3xl bg-red-500/5 dark:bg-red-500/10 border border-red-500/20 space-y-2">
        <div className="text-xs font-bold uppercase tracking-wider text-red-600 dark:text-red-400 flex items-center gap-1.5">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-muted-foreground font-semibold">
          <span className="flex items-center gap-1.5">
            <Terminal className="w-4 h-4 text-red-500" /> CLI / Architecture Code
          </span>
          <span className="font-mono text-[11px]">redis-server</span>
        </div>
        <pre className="p-5 rounded-3xl bg-slate-950 text-red-300 font-mono text-xs overflow-x-auto border border-border shadow-inner whitespace-pre-wrap leading-relaxed">
          {topic.codeSnippet}
        </pre>
      </div>
      ...
      {nextTopic && (
        <div className="pt-6 border-t border-border flex justify-end">
          <Link
            href={`/redisviz/concepts/${nextTopic.id}`}
            className="px-6 py-3 rounded-2xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs uppercase tracking-wider transition shadow-md flex items-center gap-2"
          >
