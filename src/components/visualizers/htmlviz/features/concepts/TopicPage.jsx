...
export default function TopicPage({ topicId: propTopicId }) {
    ...
    if (!tagData) return <div className="text-slate-900 dark:text-white">Topic not found</div>;
            ...
            <div className="space-y-8">
                <div className="flex items-start gap-5 border-b border-slate-200 dark:border-slate-800 pb-6">
                    <div className="w-12 h-12 bg-lime-400/10 rounded-lg flex items-center justify-center shrink-0 border border-lime-400/20">
                        <tagData.icon className="text-lime-600 dark:text-lime-400" size={24} strokeWidth={2} />
                    </div>
                    <div>
                ...
                {tagData.content && (
                    <div className="space-y-6 text-slate-700 dark:text-slate-300 leading-relaxed font-sans">
                        {tagData.content.overview && (
                            <section className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                                    <BookOpen size={20} className="text-lime-600 dark:text-lime-400" /> Overview
                                </h2>
                        ...
                        {tagData.content.syntax && (
                            <section className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                                    <Code2 size={20} className="text-lime-600 dark:text-lime-400" /> Syntax & Code Example
                                </h2>
                                <pre className="bg-slate-900 dark:bg-slate-950 text-lime-400 p-4 rounded-lg font-mono text-sm overflow-x-auto border border-slate-800">
                        ...
                        {tagData.content.ariaNotes && (
                            <section className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                                    <Sparkles size={20} className="text-lime-600 dark:text-lime-400" /> Accessibility & ARIA Guidelines
                                </h2>
