...
import { ArrowLeft, ArrowRight, BookOpen, CheckCircle2, HelpCircle, Lightbulb, RotateCcw, Sparkles } from 'lucide-react';
import { TS_FLASHCARDS, type TsFlashcard } from '../data/flashcards';
...
export default function Flashcards() {
  ...
  const filteredCards = selectedCat === 'All'
    ? TS_FLASHCARDS
    : TS_FLASHCARDS.filter((c) => c.category === selectedCat);
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-widest">
          <BookOpen className="w-3.5 h-3.5" /> Spaced Recall Trainer
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">
          TypeScript Senior Interview Flashcards
        </h1>
        <p className="text-muted-foreground text-sm sm:text-base">
          Master the tricky edge cases, variance rules, and compiler behaviors asked in top tech interviews.
        </p>
      </div>

      {/* Category Chips */}
      <div className="flex flex-wrap justify-center gap-2">
        {['All', 'Foundations', 'Generics', 'Advanced Types', 'Compiler & Architecture'].map((cat) => (
          <button
            key={cat}
            onClick={() => {
              setSelectedCat(cat);
      ...
      <div
        ...
        className="min-h-[380px] p-8 rounded-3xl border border-border bg-card hover:border-blue-500/50 shadow-xl cursor-pointer transition-all flex flex-col justify-between select-none relative group"
      >
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase">
        ...
        <div className="py-6 space-y-4">
          {!isFlipped ? (
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              {currentCard.code && (
                <pre className="p-3.5 rounded-2xl bg-slate-950 text-blue-300 font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed">
                  {currentCard.code}
                </pre>
              )}
        ...
        <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="text-blue-500 font-bold group-hover:underline">Flip Card 🔄</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-2">
        ...
        <button
          onClick={handleNext}
          className="px-6 py-2.5 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition shadow-md flex items-center gap-1.5"
        >
          <span>Next Card</span>
