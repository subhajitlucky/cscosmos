...
import { Bookmark, Check, Copy, LayoutGrid, Search } from 'lucide-react';
import { ARRAY_CHEATSHEET } from '../data/cheatsheet';
...
export default function CheatSheet() {
  ...
  const filteredSections = ARRAY_CHEATSHEET.map((sec) => ({
    ...sec,
    ...
    <div className="space-y-8 pb-20 max-w-6xl mx-auto px-4 sm:px-6 pt-10">
      <div className="space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold uppercase tracking-widest">
          <Bookmark className="w-3.5 h-3.5" /> Algorithmic Patterns Vault
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">
          Array &amp; String Algorithmic Patterns Cheat Sheet
        </h1>
        <p className="text-muted-foreground text-sm sm:text-base">
          Production-tested templates for two-pointer convergence, dynamic sliding windows, and linear KMP string searching.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search patterns (e.g. two pointer, sliding window, LPS, KMP)..."
          className="w-full pl-11 pr-4 py-3 rounded-2xl bg-card border border-border text-sm text-foreground focus:border-emerald-500 outline-none shadow-sm"
        />
      </div>
              ...
              {sec.snippets.map((snip, idx) => {
                ...
                return (
                  <div key={idx} className="p-5 rounded-3xl bg-card border border-border space-y-3 flex flex-col justify-between shadow-sm">
                    ...
                    <pre className="p-3.5 rounded-2xl bg-slate-950 text-emerald-300 font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed shadow-inner">
                      {snip.code}
                    </pre>
