...
import { ARRAY_FLASHCARDS, type ArrayFlashcard } from '../data/flashcards';
...
export default function Flashcards() {
  ...
  const filteredCards = selectedCat === 'All'
    ? ARRAY_FLASHCARDS
    : ARRAY_FLASHCARDS.filter((c) => c.category === selectedCat);
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      <div className="text-center space-y-2">
        ...
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">
          Array &amp; String Senior Interview Flashcards
        </h1>
        <p className="text-muted-foreground text-sm sm:text-base">
          Master tough DSA, Cache Locality, and String Search interview questions on pointer math, amortized aggregate method, and KMP.
        </p>
      </div>

      {/* Category Chips */}
      <div className="flex flex-wrap justify-center gap-2">
        {['All', 'Memory & Hardware Caches', 'Two-Pointer & Sliding Window', 'String Pattern Matching', 'Advanced Algorithms'].map((cat) => (
          <button
            key={cat}
            onClick={() => {
              setSelectedCat(cat);
