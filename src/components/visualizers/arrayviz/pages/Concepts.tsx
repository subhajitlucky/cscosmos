...
import { ARRAY_TOPICS, type ArrayTopic } from '../data/topics';

export default function Concepts() {
  const [filter, setFilter] = useState<string>('all');

  const filtered = filter === 'all' ? ARRAY_TOPICS : ARRAY_TOPICS.filter((t) => t.category === filter);
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto px-4 sm:px-6 pt-10">
      ...
      <div className="flex flex-wrap gap-2">
        {['all', 'memory', 'two-pointer', 'sliding-window', 'string-search', 'advanced-strings', 'optimization'].map((cat) => (
          <button
            key={cat}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((topic) => (
          <Link
            key={topic.id}
            href={`/arrayviz/concepts/${topic.id}`}
