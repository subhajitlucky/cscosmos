...
import { Search, Smartphone, BookOpen, ArrowUpRight } from 'lucide-react';
import { crossPlatformTopicGroups, crossPlatformTopics } from '../data/topics';
...
export function Learn() {
  ...
  const filteredGroups = useMemo(() => {
    return crossPlatformTopicGroups
      .map(group => {
      ...
      .filter(Boolean) as typeof crossPlatformTopicGroups;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-16">
      ...
      <div className="space-y-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[var(--cp-primary)]/30 bg-[var(--cp-primary)]/10 text-[var(--cp-primary)] text-xs font-mono">
          <BookOpen className="w-3.5 h-3.5" /> Mobile Systems Architecture Curriculum
        </div>

        <h1 className="font-display text-4xl md:text-6xl font-extrabold tracking-tight text-[var(--cp-text)]">
          The Cross-Platform <span className="text-[var(--cp-primary)] cp-glow">Architecture Map</span>
        </h1>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 pt-4">
          <div className="relative flex-grow w-full md:w-auto">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--cp-muted)]" />
            <input
              type="text"
              placeholder="Search concepts (e.g. jsi, bridge, fabric, turbomodule, hermes, flutter, impeller, kmp)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-[var(--cp-border-subtle)] bg-[var(--cp-surface)] text-xs font-mono text-[var(--cp-text)] placeholder:text-[var(--cp-muted)] focus:outline-none focus:border-[var(--cp-primary)]"
            />
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-mono whitespace-nowrap transition-colors ${
                selectedGroup === 'all'
                  ? 'bg-[var(--cp-primary)] text-white font-semibold'
                  : 'border border-[var(--cp-border-subtle)] bg-[var(--cp-surface)] text-[var(--cp-muted)] hover:text-[var(--cp-text)]'
              }`}
            >
              All ({crossPlatformTopics.length})
            </button>
            {crossPlatformTopicGroups.map((g) => (
              <button
                key={g.id}
            ...
            <div className="flex items-center justify-between border-b border-[var(--cp-border-subtle)] pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs font-bold text-[var(--cp-primary)]">
                    TRACK_0{groupIdx + 1}
                  </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {group.topics.map((topic) => (
                <Link
                  ...
                  className="cp-card p-6 rounded-xl space-y-4 group flex flex-col justify-between"
                >
                  ...
                  <div className="pt-4 border-t border-[var(--cp-border-subtle)] flex items-center justify-between">
                    <span className="text-[10px] font-mono text-[var(--cp-muted)]">
                      {topic.difficulty}
                    </span>
                    <span className="text-[10px] font-mono text-[var(--cp-primary)] flex items-center gap-1">
