...
import { API_TOPICS } from '../data/topics';

export default function ConceptDetail({ topicId }: { topicId: string }) {
  const topic = API_TOPICS.find((t) => t.id === topicId);
  ...
  const nextTopic = topic.nextTopicId
    ? API_TOPICS.find((t) => t.id === topic.nextTopicId)
    : null;
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-600 dark:text-pink-400 text-xs font-bold uppercase tracking-wider">
      ...
      <div className="p-6 rounded-3xl bg-pink-500/5 dark:bg-pink-500/10 border border-pink-500/20 space-y-2">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-muted-foreground font-semibold">
          <span className="flex items-center gap-1.5">
            <Terminal className="w-4 h-4 text-pink-500" /> Architecture Blueprint &amp; Code
          </span>
          <span className="font-mono text-[11px]">Production Standard</span>
        </div>
        <pre className="p-5 rounded-3xl bg-slate-950 text-pink-300 font-mono text-xs overflow-x-auto border border-border shadow-inner whitespace-pre-wrap leading-relaxed">
          {topic.codeSnippet}
        </pre>
      </div>
      ...
      <div className="p-6 rounded-3xl bg-card border border-border space-y-4">
        <h3 className="text-base font-bold text-foreground flex items-center gap-2">
          <CheckCircle2 className="w-4 h-4 text-pink-500" /> Key Architectural Takeaways
        </h3>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="p-5 rounded-2xl bg-rose-500/10 border border-rose-500/20 space-y-2">
          <div className="text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400 flex items-center gap-1.5">
            <XCircle className="w-4 h-4" /> Common Architectural Pitfall
          </div>
        ...
        <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 space-y-2">
          <div className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
            <CheckCircle2 className="w-4 h-4" /> Production Best Practice
          </div>
      ...
      {nextTopic && (
        <div className="pt-6 border-t border-border flex justify-end">
          <Link
            href={`/apiviz/concepts/${nextTopic.id}`}
            className="px-6 py-3 rounded-2xl bg-pink-600 hover:bg-pink-700 text-white font-bold text-xs uppercase tracking-wider transition shadow-md flex items-center gap-2"
          >
