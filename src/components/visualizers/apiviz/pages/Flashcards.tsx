...
import { API_FLASHCARDS, type ApiFlashcard } from '../data/flashcards';
...
export default function Flashcards() {
  ...
  const filteredCards = selectedCat === 'All'
    ? API_FLASHCARDS
    : API_FLASHCARDS.filter((c) => c.category === selectedCat);
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      <div className="text-center space-y-2">
        ...
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">
          Senior API Architect Flashcards
        </h1>
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {['All', 'GraphQL Internals', 'REST & HTTP Standards', 'gRPC & Microservices', 'API Security & Resilience'].map((cat) => (
          <button
            key={cat}
            onClick={() => {
              setSelectedCat(cat);
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${
              selectedCat === cat
                ...
                : 'bg-card border border-border text-foreground hover:border-pink-500'
            }`}
          >
            {cat}
          </button>
      ...
      <div
        ...
        className="min-h-[360px] p-8 rounded-3xl border border-border bg-card hover:border-pink-500/50 shadow-xl cursor-pointer transition-all flex flex-col justify-between select-none relative group"
      >
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-pink-500/10 text-pink-600 dark:text-pink-400 text-xs font-bold uppercase">
          ...
          {!isFlipped ? (
            <div className="space-y-3">
              <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                <HelpCircle className="w-4 h-4 text-pink-500" /> Question:
              </div>
            ...
            <div className="space-y-4 animate-in fade-in">
              <div className="text-xs font-bold uppercase tracking-wider text-pink-600 dark:text-pink-400 flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-pink-500" /> Answer:
              </div>
              <p className="text-sm sm:text-base text-foreground leading-relaxed font-medium">
                {currentCard.answer}
              </p>
              {currentCard.code && (
                <pre className="p-3.5 rounded-2xl bg-slate-950 text-pink-300 font-mono text-xs overflow-x-auto whitespace-pre-wrap leading-relaxed">
                  {currentCard.code}
                </pre>
              )}
        ...
        <div className="pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="text-pink-500 font-bold group-hover:underline">Flip Card 🔄</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-2">
        ...
        <button
          onClick={handleNext}
          className="px-6 py-2.5 rounded-2xl bg-pink-600 hover:bg-pink-700 text-white text-xs font-bold transition shadow-md flex items-center gap-1.5"
        >
          <span>Next Card</span>
