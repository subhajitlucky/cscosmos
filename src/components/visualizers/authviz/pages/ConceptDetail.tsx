...
import { AUTH_TOPICS } from '../data/topics';

export default function ConceptDetail({ topicId }: { topicId: string }) {
  const topic = AUTH_TOPICS.find((t) => t.id === topicId);
  ...
  const nextTopic = topic.nextTopicId
    ? AUTH_TOPICS.find((t) => t.id === topic.nextTopicId)
    : null;
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-muted-foreground font-semibold">
          <span className="flex items-center gap-1.5">
            <Terminal className="w-4 h-4 text-emerald-500" /> Architecture Blueprint &amp; Code
          </span>
      ...
      {nextTopic && (
        <div className="pt-6 border-t border-border flex justify-end">
          <Link
            href={`/authviz/concepts/${nextTopic.id}`}
