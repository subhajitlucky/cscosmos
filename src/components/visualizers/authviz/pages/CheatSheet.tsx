...
import { Bookmark, Check, Copy, KeyRound, Search, ShieldCheck } from 'lucide-react';
import { AUTH_CHEATSHEET } from '../data/cheatsheet';
...
export default function CheatSheet() {
  ...
  const filteredSections = AUTH_CHEATSHEET.map((sec) => ({
    ...sec,
    ...
    <div className="space-y-8 pb-20 max-w-6xl mx-auto px-4 sm:px-6 pt-10">
      <div className="space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold uppercase tracking-widest">
          <Bookmark className="w-3.5 h-3.5" /> Security Patterns Vault
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tight">
          Auth &amp; Identity Architecture Cheat Sheet
        </h1>
        <p className="text-muted-foreground text-sm sm:text-base">
          Production-tested templates for Web Crypto PKCE generators, RS256 JWKS clients, and Argon2id hashing.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search patterns (e.g. PKCE, JWKS, RS256, argon2, cookies)..."
