...
import { Search, Radio, BookOpen, ArrowUpRight } from 'lucide-react';
import { mqTopicGroups, mqTopics } from '../data/topics';
...
export function Learn() {
  ...
  const filteredGroups = useMemo(() => {
    return mqTopicGroups
      ...
      .filter(Boolean) as typeof mqTopicGroups;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-16">
      ...
      <div className="space-y-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[var(--mq-primary)]/30 bg-[var(--mq-primary)]/10 text-[var(--mq-primary)] text-xs font-mono">
          <BookOpen className="w-3.5 h-3.5" /> Message Queues &amp; Streaming Curriculum
        </div>

        <h1 className="font-display text-4xl md:text-6xl font-extrabold tracking-tight text-[var(--mq-text)]">
          The Event Streaming <span className="text-[var(--mq-primary)] mq-glow">Architecture Map</span>
        </h1>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 pt-4">
          <div className="relative flex-grow w-full md:w-auto">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--mq-muted)]" />
            <input
              type="text"
              placeholder="Search messaging concepts (e.g. partition, rebalance, outbox, DLQ)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-[var(--mq-border-subtle)] bg-[var(--mq-surface)] text-xs font-mono text-[var(--mq-text)] placeholder:text-[var(--mq-muted)] focus:outline-none focus:border-[var(--mq-primary)]"
            />
          </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-mono whitespace-nowrap transition-colors ${
                selectedGroup === 'all'
                  ? 'bg-[var(--mq-primary)] text-black font-semibold'
                  : 'border border-[var(--mq-border-subtle)] bg-[var(--mq-surface)] text-[var(--mq-muted)] hover:text-[var(--mq-text)]'
              }`}
            >
              All ({mqTopics.length})
            </button>
            ...
            <div className="flex items-center justify-between border-b border-[var(--mq-border-subtle)] pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs font-bold text-[var(--mq-primary)]">
                    TRACK_0{groupIdx + 1}
                  </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {group.topics.map((topic) => (
                <Link
                  ...
                  className="mq-card p-6 rounded-xl space-y-4 group flex flex-col justify-between"
                >
                  ...
                  <div className="pt-4 border-t border-[var(--mq-border-subtle)] flex items-center justify-between">
                    <span className="text-[10px] font-mono text-[var(--mq-muted)]">
                      {topic.difficulty}
                    </span>
                    <span className="text-[10px] font-mono text-[var(--mq-primary)] flex items-center gap-1">
