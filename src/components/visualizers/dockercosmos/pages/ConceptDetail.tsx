...
import { DOCKER_TOPICS } from '../data/topics';

export default function ConceptDetail({ topicId }: { topicId: string }) {
  const topic = DOCKER_TOPICS.find((t) => t.id === topicId);
  ...
  const nextTopic = topic.nextTopicId
    ? DOCKER_TOPICS.find((t) => t.id === topic.nextTopicId)
    : null;
    ...
    <div className="space-y-8 pb-20 max-w-4xl mx-auto px-4 sm:px-6 pt-10">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/20 text-sky-600 dark:text-sky-400 text-xs font-bold uppercase tracking-wider">
      ...
      <div className="p-6 rounded-3xl bg-sky-500/5 dark:bg-sky-500/10 border border-sky-500/20 space-y-2">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-muted-foreground font-semibold">
          <span className="flex items-center gap-1.5">
            <Terminal className="w-4 h-4 text-sky-500" /> Dockerfile / YAML Manifest / CLI
          </span>
          <span className="font-mono text-[11px]">Production Standard</span>
        </div>
        <pre className="p-5 rounded-3xl bg-slate-950 text-sky-300 font-mono text-xs overflow-x-auto border border-border shadow-inner whitespace-pre-wrap leading-relaxed">
          {topic.codeSnippet}
        </pre>
      </div>
      ...
      {nextTopic && (
        <div className="pt-6 border-t border-border flex justify-end">
          <Link
            href={`/dockercosmos/concepts/${nextTopic.id}`}
            className="px-6 py-3 rounded-2xl bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs uppercase tracking-wider transition shadow-md flex items-center gap-2"
          >
